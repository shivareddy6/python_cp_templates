...
class LazySegmentTree:
    ...
    def update(self, alpha, omega, value):
        """Increases all elements in the range alpha to omega (inclusive) by given value!"""
        alpha += self.size
        omega += self.size + 1
        l, r = alpha, omega
        while alpha < omega:
...
arr = [1, 5, 2, 3]
seg = LazySegmentTree(arr, func = lambda a,b: a+b) # for min just give fun=min

print("sum of elements between 1 and 3 is:", seg.query(1, 3))
seg.update(1, 2, 2) #increase all elements between 1 and 2 by 2
print("sum of elements between 1 and 3 is:", seg.query(1, 3))